...
export function CompanyInternships() {
  ...
  const load = async (filters = { search, semesterId, skillId: filterSkillId, status }) => {
    ...
    setError(null);
    try {
      const [page, skillPage, activeSemesterPage] = await Promise.all([
