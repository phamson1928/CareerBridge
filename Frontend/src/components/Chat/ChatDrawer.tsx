      ...
      <aside className="flex h-full w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-2xl">
        <header className="flex items-center justify-between border-b border-slate-200 bg-slate-900 p-4 text-white">
          <div className="flex items-center gap-3">
            {selected && (
              <button onClick={() => setSelected(null)} aria-label="Quay lại">
                <ArrowLeft className="h-5 w-5" />
              </button>
            )}
            {selected ? (
              <ProfileAvatarPreview
                fileId={selected.participant.avatarFileId}
                fallback={selected.participant.name.charAt(0).toUpperCase()}
                className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-blue-500 text-xs font-bold"
            ...
            <div>
              <h3 className="text-sm font-bold">
                {selected?.participant.name ?? "Tin nhắn"}
              </h3>
              <p className="text-[11px] text-slate-300">
                {selected?.internship?.title ?? "Hội thoại thực tập"}
              </p>
            </div>
          </div>
          <button
        ...
        {!selected ? (
          <div className="flex-1 overflow-y-auto bg-slate-50 p-3">
            ...
            {!isLoading && supervisedPlacements.length > 0 && (
              <section className="mb-4 rounded-xl border border-indigo-100 bg-indigo-50 p-3">
                <p className="mb-2 text-xs font-bold text-indigo-900">
                  {user?.role === "LECTURER"
                    ? "Trao đổi với sinh viên phụ trách"
                    : "Trao đổi với giảng viên hướng dẫn"}
                </p>
                {supervisedPlacements.map((placement) => {
                  const person =
                    user?.role === "LECTURER"
                      ? placement.student
                      : placement.supervision?.lecturer;
                  return (
                    <button
                      key={placement.id}
                      onClick={() => void startLecturerConversation(placement)}
                      className="mb-2 flex w-full items-center gap-2 rounded-lg bg-white p-2.5 text-left text-xs font-semibold text-slate-700 shadow-sm hover:bg-indigo-100"
                    >
                      <ProfileAvatarPreview
                        fileId={person?.avatarFileId}
                        fallback={
                          person?.fullName.charAt(0).toUpperCase() ?? "G"
                        }
                        className="flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-full bg-indigo-100 text-xs font-bold text-indigo-700"
                        imageClassName="h-full w-full object-cover"
                      />
                      <span className="min-w-0 flex-1 truncate">
                        {person?.fullName} · {placement.internship.title}
                      </span>
                    </button>
                  );
                })}
            ...
            {conversations.map((conversation) => (
              <button
                ...
                className="mb-2 flex w-full items-start gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left shadow-sm hover:border-blue-300"
              >
                <ProfileAvatarPreview
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50 p-4">
              {isLoading ? (
                ...
                messages.map((message) => (
                  <div
                    key={message.id}
                    className={`mb-3 flex flex-col ${message.senderId === user?.id ? "items-end" : "items-start"}`}
                  >
                    <p
