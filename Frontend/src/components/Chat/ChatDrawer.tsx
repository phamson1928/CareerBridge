...
import { ArrowLeft, GraduationCap, MessageSquare, RefreshCw, Send, X } from 'lucide-react';
import { useAuth } from '../../auth/AuthContext';
import { getApiErrorMessage } from '../../auth/api';
import { createConversation, listConversations, listMessages, markConversationMessagesRead, sendMessage } from '../../chat/api';
...
export function ChatDrawer({ isOpen, onClose, latestMessage, onMessagesRead }: ChatDrawerProps) {
  ...
  const [supervisedPlacements, setSupervisedPlacements] = useState<PlacementRecord[]>([]);
  ...
  const loadConversations = async () => {
    ...
    try {
      const [conversationPage, placementPage] = await Promise.all([
        ...
        user?.role === 'STUDENT' || user?.role === 'LECTURER' ? placementsApi.listMine() : Promise.resolve(null),
      ]);
      setConversations(conversationPage.items);
      setSupervisedPlacements((placementPage?.items ?? []).filter((placement) => placement.supervision?.status === 'ACTIVE'));
    }
  ...
  const startLecturerConversation = async (placement: PlacementRecord) => {
    setIsLoading(true); setError(null);
    try {
      const conversation = await createConversation({ placementId: placement.id });
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/30 backdrop-blur-xs"><aside className="flex h-full w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-2xl">
    <header className="flex items-center justify-between border-b border-slate-200 bg-slate-900 p-4 text-white">
      <div className="flex items-center gap-3">{selected && <button onClick={() => setSelected(null)} aria-label="Quay lại"><ArrowLeft className="h-5 w-5" /></button>}<MessageSquare className="h-5 w-5" /><div><h3 className="text-sm font-bold">{selected?.participant.name ?? 'Tin nhắn'}</h3><p className="text-[11px] text-slate-300">{selected?.internship?.title ?? 'Hội thoại thực tập'}</p></div></div>
