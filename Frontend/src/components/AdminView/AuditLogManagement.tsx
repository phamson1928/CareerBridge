...
export const AuditLogManagement: React.FC = () => {
  ...
  return (
    <section className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm text-slate-300">
                  Theo dõi các thay đổi quan trọng trong hệ thống. Metadata nhạy
                  cảm đã được backend che trước khi trả về.
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 gap-3 p-5 sm:grid-cols-2 lg:grid-cols-5 sm:p-6">
          ...
          <FilterInput label="Action">
            <input
              ...
              placeholder="Ví dụ: USER_UPDATED"
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none transition focus:border-indigo-300 focus:bg-white focus:ring-2 focus:ring-indigo-100"
            />
          </FilterInput>
          ...
          <div className="flex items-end gap-2 lg:col-span-5">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold text-slate-600 transition hover:bg-slate-50"
            >
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-3 py-2.5 text-xs font-bold text-white transition hover:bg-indigo-700 disabled:opacity-60"
            >
              <RefreshCw
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xs">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 sm:px-6">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Nhấn vào một dòng để xem metadata đã được redaction.
            </p>
          </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-5 py-14 text-center">
                    <FileSearch className="mx-auto mb-2 h-7 w-7 text-slate-300" />
                    <p className="font-semibold text-slate-700">
                      Không tìm thấy hoạt động phù hợp.
                    </p>
                    <p className="mt-1 text-slate-500">
                      Thử điều chỉnh bộ lọc hoặc tải lại dữ liệu.
                    </p>
                  </td>
                </tr>
              ) : (
                logs.map((log) => (
                  <tr key={log.id} className="transition hover:bg-indigo-50/40">
                    ...
                    <td className="px-5 py-4">
                      <span className="inline-flex rounded-lg bg-indigo-50 px-2 py-1 font-mono text-[11px] font-bold text-indigo-700">
                        {log.action}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      ...
                      {log.entityId && (
                        <p className="mt-0.5 max-w-36 truncate font-mono text-[10px] text-slate-400">
                          {log.entityId}
                        </p>
                      )}
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <button
                        ...
                        className="rounded-lg px-2.5 py-1.5 font-bold text-indigo-700 transition hover:bg-indigo-100"
                      >
                        Xem
                      </button>
                    </td>
        ...
        <footer className="flex flex-col gap-3 border-t border-slate-100 px-5 py-4 text-xs sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <span className="text-slate-500">
            {new Intl.NumberFormat("vi-VN").format(total)} bản ghi
          </span>
          <div className="flex items-center gap-2">
            <button
              disabled={loading || page <= 1}
              onClick={() => setPage((value) => value - 1)}
              className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 font-bold text-slate-600 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <ChevronLeft className="h-3.5 w-3.5" /> Trước
            </button>
            <span className="min-w-24 text-center font-semibold text-slate-600">
      ...
      {selectedLog && (
        <div
          className="fixed inset-0 z-[60] flex justify-end bg-slate-950/35 p-0 backdrop-blur-[1px]"
          role="dialog"
          aria-modal="true"
            ...
            <div className="flex-1 space-y-6 overflow-y-auto p-5">
              {detailLoading && (
                <p className="text-sm text-slate-500">
                  Đang tải chi tiết mới nhất...
                </p>
              )}
              <div className="grid grid-cols-2 gap-3">
                <DetailItem label="Action" value={selectedLog.action} mono />
                <DetailItem label="Entity" value={selectedLog.entity} />
                <DetailItem
                  label="Thời điểm"
                ...
                <DetailItem
                  label="IP"
                  value={selectedLog.ipAddress ?? "Không có"}
                  mono
                />
                <DetailItem
                  label="Actor"
                  value={selectedLog.actor?.email ?? "Hệ thống"}
                />
                <DetailItem
                  label="Vai trò"
                  value={formatRole(selectedLog.actor?.role)}
                />
              </div>
              <DetailItem
                label="Entity ID"
                value={selectedLog.entityId ?? "Không có"}
                mono
              />
              <div>
                ...
                <pre className="max-h-[48vh] overflow-auto rounded-2xl bg-slate-950 p-4 text-xs leading-5 text-slate-100">
                  {getMetadataText(selectedLog.metadata)}
                </pre>
              </div>
            </div>
          </aside>
...
const DetailItem: React.FC<{
  ...
  <div className="min-w-0 rounded-xl bg-slate-50 p-3">
    ...
    <p
      className={`mt-1 break-all text-xs font-semibold text-slate-700 ${mono ? "font-mono" : ""}`}
    >
      {value}
    </p>
  </div>
