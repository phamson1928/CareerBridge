...
const CompanyLogo: React.FC<{ src: string; name: string; className: string }> = ({ src, name, className }) =>
  src ? (
    ...
    <div className={`${className} flex items-center justify-center bg-indigo-50 text-sm font-black text-indigo-600`}>
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2 text-slate-800 focus-within:border-indigo-500 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
            ...
            <input
              ...
              placeholder="Tìm theo vị trí (Backend Developer, React, Tester, Data Analyst...)"
      ...
      {internshipsError && (
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
          Không thể tải vị trí thực tập: {internshipsError}
        </div>
      )}
      ...
      {isLoadingInternships && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {[1, 2, 3, 4].map((skeletonId) => (
            <div key={skeletonId} className="animate-pulse bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-4">
      ...
      {!isLoadingInternships && !internshipsError && filteredInternships.length === 0 && (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">
          Không có vị trí thực tập phù hợp.
        </div>
      )}
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-4 text-xs text-slate-600">
                    ...
                    <span className="flex items-center gap-1 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-200">
                      <Calendar className="w-3.5 h-3.5 text-slate-400" /> Hạn: {formatDate(job.deadline, "Không thời hạn")}
                    </span>
                  </div>
      ...
      {internshipPage.pagination.totalPages > 1 && (
        <div className="flex items-center justify-center gap-3 text-sm">
          <button
            type="button"
            disabled={isLoadingInternships || page <= 1}
            onClick={() => setPage((value) => value - 1)}
            className="rounded-xl border border-slate-300 px-4 py-2 font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Trang trước
          </button>
          <span className="text-slate-600">
            Trang {page} / {internshipPage.pagination.totalPages}
          </span>
          <button
            type="button"
            disabled={isLoadingInternships || page >= internshipPage.pagination.totalPages}
            onClick={() => setPage((value) => value + 1)}
            className="rounded-xl border border-slate-300 px-4 py-2 font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Trang sau
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl relative animate-in fade-in zoom-in duration-150">
            ...
            <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
              <CompanyLogo
                ...
                name={activeModalInternship.companyName}
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                  ...
                  <span>📅 Hạn: {formatDate(activeModalInternship.deadline, "Không thời hạn")}</span>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleApplySubmit} className="space-y-4 text-xs">
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-xs flex items-center gap-2 disabled:opacity-50"
                >
                  {isSubmitting && <Loader2 className="w-4 h-4 animate-spin" />}
                  <span>{isSubmitting ? 'Đang nộp...' : 'Xác nhận nộp đơn'}</span>
                </button>
              </div>
